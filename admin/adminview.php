...
<?php /************************************************************/
  ...
  {
/********************************************************** PHP */?>
<td><a href="#attach"><img src="./images/ticket-attach.png" alt="Attach File" border="0" hspace="5" valign="middle" /></a></td><td><div class="normal"><a href="#attach">Attach File</a>&nbsp;&nbsp;&nbsp;</div></td>
<?php /************************************************************/
  }
/********************************************************** PHP */?>
<td><a href="javascript:if(confirm('Are you sure?')) window.location.href = '<?php echo $HD_CURPAGE . "?cmd=delete&id={$_GET[id]}" ?>'"><img src="./images/ticket-delete.png" alt="Delete<?php if( $is_ticket ) echo " Ticket" ?>" border="0" hspace="5" valign="middle" /></a></td><td><div class="normal"><a href="javascript:if(confirm('Are you sure?')) window.location.href = '<?php echo $HD_CURPAGE . "?cmd=delete&id={$_GET[id]}" ?>'">Delete<?php if( $is_ticket ) echo " Ticket" ?></a>&nbsp;&nbsp;&nbsp;</div></td>
<td><a href="#cc"><img src="./images/ticket-mailcc.png" alt="Carbon Copy" border="0" hspace="5" valign="middle" /></a></td><td><div class="normal"><a href="#cc">Carbon Copy</a>&nbsp;&nbsp;&nbsp;</div></td>
<?php /************************************************************/
  if( $is_ticket && mysql_num_rows( $res_others ) )
  {
/********************************************************** PHP */?>
<td><a href="#others"><img src="./images/ticket-history.png" alt="Other Tickets" border="0" hspace="5" valign="middle" /></a></td><td><div class="normal"><a href="#others">Client History</a>&nbsp;&nbsp;&nbsp;</div></td>
<?php /************************************************************/
  }
/********************************************************** PHP */?>
</tr>
...
<?php /************************************************************/
  ...
  {
/********************************************************** PHP */?>
<img src="./images/blank.gif" width="1" height="8" /><br />
<center>
<table border="0" cellspacing="0" cellpadding="0">
...
<tr>
<td align="right">
<div class="normal"><img src="./images/mail-flag.png" /> Flag For:&nbsp;
<select name="flag">
...
<?php /************************************************************/
    if( trim( $row[custom] ) != "" )
    {
      ...
      echo "<tr bgcolor=\"#3c91c7\"><td colspan=\"2\"><div class=\"normal\">Custom Fields:</div></td></tr>";
...
<?php /************************************************************/
  ...
  while( $row_post = mysql_fetch_array( $res_post ) )
  {
    ...
    if( !$row_post["private"] )
    {
      $bgcolor = ($bgcolor == "#e4f2fd") ? "#EEEEEE" : "#e4f2fd";
    ...
    if( $row_post[user_id] == -1 )
    {
      ...
      echo "<table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\"><tr><td><div class=\"normal\"><font size=\"1\"><b>Subject:</b> " . field( $row_post[subject] ) . "</font></div></td><td><img src=\"./images/blank.gif\" width=\"10\" height=\"1\" /></td><td align=\"right\"><div class=\"normal\"><font size=\"1\"><b>Posted by <a href=\"mailto:{$row[email]}\">" . field( $row[name] ) . "</a>$ip</b></font></div></td></tr></table>";
    }
    else
    {
    ...
    if( ($is_ticket && $priv && ($row_post[id] != $first_id)) || (!$is_ticket && ($row_post[user_id] == $_SESSION[user][id]) && ($row_post[id] != $first_id)) )
      echo "<a href=\"javascript:if(confirm('Are you sure you want to delete this post?')) window.location.href = '{$HD_CURPAGE}?cmd=deletepost&postid={$row_post[id]}&id={$_GET[id]}'\"><img src=\"./images/ticket-delete.png\" border=\"0\" alt=\"Delete Post\" /></a></td><td><div class=\"normal\"><font size=\"1\"><a href=\"javascript:if(confirm('Are you sure you want to delete this post?')) window.location.href = '{$HD_CURPAGE}?cmd=deletepost&postid={$row_post[id]}&id={$_GET[id]}'\">Delete Post</a></font><img src=\"./images/blank.gif\" width=\"10\" height=\"1\" /></div>";
      
    echo "</td><td align=\"right\"><div class=\"normal\"><font size=\"1\">Date: " . date( "m-j-Y g:ia T", $row_post[date] ) . "</font></div></td></tr></table>";
  }
/********************************************************** PHP */?>
</table>
<br />
...
/********************************************************** PHP */?>
<div id="container">
<h1>Predefined Reply</h1>
<form class="wufoo" name="predefineddelete" action="<?php echo $HD_CURPAGE ?>" method="post">
	<ul>
<li>
	   <label class="desc">Predefined Reply:</label>
<input type="hidden" name="id" value="<?php echo $_GET[id] ?>" />
<input type="hidden" name="replyname" value="" />
<input type="hidden" name="cmd" value="deletereply" />
<span>
<select class="field select" name="reply" onchange="document.predefinedreply.message.value = this.options[selectedIndex].value; if( this.options[selectedIndex].value != '' )  { document.predefinedreply.replyname.value = this.options[selectedIndex].text; document.predefineddelete.replyname.value = this.options[selectedIndex].text; } else { document.predefinedreply.replyname.value = ''; document.predefineddelete.replyname.value = ''; }">
...
<div class="buttons">
    <button type="submit" class="negative">Delete Selected Reply</button>
</div>
</form>
</div>
<?php /************************************************************/
...
<div id="container">
	<h1>Post Reply</h1>
<form class="wufoo" name="predefinedreply" action="<?php echo $HD_CURPAGE ?>" method="post">
<input type="hidden" name="id" value="<?php echo $_GET[id] ?>" />
...
<form class="wufoo" enctype="multipart/form-data" action="<?php echo $HD_CURPAGE ?>" method="post">
  ...
  <input type="hidden" name="id" value="<?php echo $_GET[id] ?>" />
  <li>
	   <label class="desc">Attach File:</label> <input name="userfile" type="file"> <input type="submit" value="Attach">
</form>
</div>
<?php /************************************************************/
  }
...
<a name="#cc"></a>
<div id="container">
	<h1>Carbon Copy Replies</h1>
<form class="wufoo" action="<?php echo $HD_CURPAGE ?>" method="post">
<input type="hidden" name="cmd" value="cc" />
<input type="hidden" name="id" value="<?php echo $_GET[id] ?>" />
<li>
	   <label class="desc">Email Addresses:</label>
	   <div>
	    <input type="text" name="cc" size="40" value="<?php echo field( $row[cc] ) ?>" />
...
<br />
<a name="#others"></a>
<div class="title">Client History</div><br />
