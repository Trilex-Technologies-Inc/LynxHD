...
<div class="card shadow-sm mb-4"><div class="card-header font-weight-bold">Embed chat on another website</div><div class="card-body"><p class="text-muted">Paste this code before the closing <code>&lt;/body&gt;</code> tag on your website.</p><div class="input-group"><textarea class="form-control" id="livechat-embed-code" rows="2" readonly><?php echo htmlspecialchars($livechat_embed_code, ENT_QUOTES, 'UTF-8') ?></textarea><div class="input-group-append"><button class="btn btn-outline-primary" type="button" id="copy-livechat-embed">Copy code</button></div></div><small class="form-text text-muted">Add <code>data-position=&quot;left&quot;</code> to the script tag to place the chat on the left.</small></div></div>
<script>document.getElementById('copy-livechat-embed').addEventListener('click',function(){var button=this,code=document.getElementById('livechat-embed-code');function copied(){button.textContent='Copied';setTimeout(function(){button.textContent='Copy code'},1500)}if(navigator.clipboard&&window.isSecureContext){navigator.clipboard.writeText(code.value).then(copied)}else{code.select();document.execCommand('copy');copied()}});</script>
<?php endif; ?>
