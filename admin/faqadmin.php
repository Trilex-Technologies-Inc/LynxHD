  ...
  if( $global_priv )
  {
    if( trim( $_POST['name'] ?? '' ) != "" )
    {
      if( !get_row_count( "SELECT COUNT(*) FROM {$pre}faq WHERE ( parent = '{$_POST['parent']}' && description = '{$_POST['name']}' )" ) )
      {
        ...
        if( $category_created )
        {
          Header( "Location: {$HD_CURPAGE}?parent=" . urlencode( $_POST['parent'] ) );
          exit;
        }
        else
...
?>
  ...
  <?php $res = mysql_query("SELECT id, description, symptoms FROM {$pre}faq WHERE ( parent = '{$_POST['parent']}' ) ORDER BY description"); ?>
  <?php if(mysql_num_rows($res)): ?><h2 class="h5 mb-3">Categories</h2><div class="row mb-4"><?php while($row = mysql_fetch_array($res)): $items = get_row_count("SELECT COUNT(*) FROM {$pre}faq WHERE ( category = '{$row['id']}' )"); $subcats = get_row_count("SELECT COUNT(*) FROM {$pre}faq WHERE ( parent = '{$row['id']}' )"); ?><div class="col-md-6 mb-3"><a class="card shadow-sm h-100 text-decoration-none" href="<?php echo field($HD_CURPAGE) ?>?parent=<?php echo (int)$row['id'] ?>"><div class="card-body"><h3 class="h6 text-primary"><?php echo field($row['description']) ?></h3><p class="text-muted small mb-2"><?php echo trim($row['symptoms']) !== '' ? field($row['symptoms']) : 'No description' ?></p><span class="badge badge-light mr-1"><?php echo $subcats ?> subcategories</span><span class="badge badge-light"><?php echo $items ?> articles</span></div></a></div><?php endwhile; ?></div><?php endif; ?>
...
<?php elseif($faq_cmd == 'search'):
  $search = $_GET['search'] ?? '';
  $res = mysql_query("SELECT * FROM {$pre}faq WHERE ( description LIKE '%$search%' || symptoms LIKE '%$search%' || solution LIKE '%$search%' ) ORDER BY date DESC");
?>
